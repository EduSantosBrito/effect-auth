...
export const hashTokenValue = (
  ...
  Schema.decodeUnknownEffect(TokenHash)(sha256(Redacted.value(token))).pipe(
...
> =>
  Effect.gen(function* () {
    const token = yield* Effect.try({
      ...
      Effect.flatMap((value) => Schema.decodeUnknownEffect(VerificationToken)(value)),
...
> =>
  Effect.gen(function* () {
    const token = yield* Effect.try({
      ...
      Effect.flatMap((value) => Schema.decodeUnknownEffect(SessionToken)(value)),
      Effect.mapError(() => new TokenGenerationFailure({ reason: "UnavailableEntropy" })),
    );
    const hash = yield* hashTokenValue(token);
    return { token, hash };
